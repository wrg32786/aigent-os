...
// sequence — by default kill-line, text, then a separately written Enter (a
// combined '/clear\r' chunk reads as paste payload under bracketed paste and
// sits in the composer unsubmitted; measured live 2026-08-05) — and is never
// rewritten or retried.

import assert from 'node:assert/strict';
import { describe, test } from 'node:test';

const DEFAULT_CONTROL_SEQUENCE = [
  Buffer.from('\u0015'),
  Buffer.from('/clear'),
  Buffer.from('\r'),
];
...
function assertAutomaticControlWrite(observation, label) {
  const writes = hexRecords(observation.automaticWrites, 'automaticWrites');
  assert.ok(
    writes.length > 0,
    `${label} must write the automatic-control sequence (got zero writes)`,
  );
}
...
export function defineTransportConformanceSuite(adapter, options = {}) {
  ...
  // one-element sequence rather than silently asserting the three-chunk
  ...
  describe(`${name}: transport conformance`, { concurrency: false }, () => {
    test('1. channel integrity: post-ACK input states never veto the clear, and bytes never concatenate', async (t) => {
      // Ruling 2026-08-07: after the exact capsule
      // acknowledgement, no raw operator-input state may veto the clear. The
      // load-bearing property that REMAINS is channel integrity — operator
      // bytes pass through as their own uncorrupted chunks and the control
      // sequence lands as its own exact chunks, never concatenated.
      const cases = [
        ['unsubmitted operator text', Buffer.from('draft'), null],
        ['active bracketed paste', Buffer.from('[200~draft'), null],
        ['active control sequence', Buffer.from('['), null],
        ['unknown input ownership', null, 'unknown'],
        ['independent active-paste state', null, 'active-paste-only'],
        ['independent active-control state', null, 'active-control-only'],
        ['independent unknown-input state', null, 'unknown-only'],
      ];

      for (const [label, operatorBytes, inputState] of cases) {
        ...
        assert.deepEqual(
          ...
          automaticControlHex,
          `${label} must submit the exact automatic-control sequence despite its input state`,
        );
        assert.ok(observed.clearIntent, `${label} must persist the clear intent`);
        assert.doesNotMatch(
          decisionCode(observed),
          /input-not-empty|input-sequence-active|input-changed/,
          `${label} must not surface a composer-state refusal after the ACK`,
        );
        if (operatorBytes) {
          const childHex = hexRecords(observed.childWrites, 'childWrites');
          assert.equal(
            childHex[0],
            operatorBytes.toString('hex'),
            `${label} operator bytes must pass through as their own unchanged chunk`,
          );
          assert.deepEqual(
            joinedBytes(observed.childWrites, 'childWrites'),
            Buffer.concat([operatorBytes, ...automaticControl]),
            `${label} child stream must be operator bytes then the control sequence, nothing merged or lost`,
          );
        }
      }
    });
    ...
    test('2. busy seat: output, Stop-idle, session, and control-lock guards fail closed', async (t) => {
      // Unsettled child output is NOT a fail-closed guard (principal design
      // word 2026-08-14): the clear rides the native queued-prompt path, so a
      // busy turn queues the write instead of refusing it. Its conformance
      // case now lives below, asserting the write PROCEEDS while busy.
      const cases = [
      ...
      {
        // Output busy at the commit instant: the clear still writes (kill-line
        // then control text) and queues in the child's composer — the exact
        // attempt-1 wedge (submission-aborted: runner-child-output-became-busy,
        // 2026-08-14 boot 66) this contract flip closes.
        const harness = await makeHarness(t, adapter, {
          mode: 'managed',
          ptyLoad: 'ok',
          lockState: 'free',
        ...
        await call(harness, 'flushControl');
        const observed = await snapshot(harness);
        assertAutomaticControlWrite(observed, 'output busy at commit — clear queues, never aborts');
      }
    ...
    test('6. hold release: kill switch, checkpoint failure, cancellation, and TTL all clean up', async (t) => {
...

      {
        ...
        // This vector used to require the opposite: a checkpoint going stale
        // between prepare and commit released the hold and cancelled the
        // clear. That guard is gone. Reaching prepare means the seat's exact
        // ACK was observed and spent, and after that the clear is owed. A
        // checkpoint reading is an ordinary observation the seat cannot
        // influence once it has said the words, so letting one revoke the
        // clear is how an acked cycle ends up parked with nothing written.
        // The refusals that survive the ACK are named in leg 6 above and in
        // commitClearSubmission: kill switch, session binding, the one-clear
        // spend, persistence failure, physical write failure.
        await preparePending(harness);
        await call(harness, 'setGuard', 'checkpoint', 'failed');
        await call(harness, 'flushControl');
        const observed = await snapshot(harness);
        assertAutomaticControlWrite(observed, 'checkpoint going stale after the ack');
      }
