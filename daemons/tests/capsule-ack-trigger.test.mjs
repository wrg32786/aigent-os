...
// FRESHNESS and for nothing else. The event-driven transport design,
...
test('a stale reading with a number passes — context pressure is monotonic, staleness holds are deleted', () => {
  // The accepted transport rule, 2026-08-05: no timers. A stale reading can only
...
test('LAW XV — each bound can independently go red', () => {
  ...
  // 2) An AGED ack still stands — the accepted transport rule (2026-08-05)
...
// ACCEPTED TRANSPORT RULE: the exact capsule completion acknowledgement
// is cycle-scoped and does not expire. Capsule work can take several
// minutes, while context usage only grows inside a session; a stale
// numeric pressure reading can only understate usage.
...
test('post-ack, the submission gate consults no telemetry clock', () => {
  ...
  fs.mkdirSync(path.join(root, 'runtime'), { recursive: true });
  // The capsule EXISTS and binds — the checkpoint contract, kept with teeth:
