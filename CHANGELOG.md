...
- **Semantic-search confidential-class deny list**: `daemons/semantic-search/index-deny.json`, a declarative list of vault-relative path prefixes that `embed-vault.js` (index-build time) and `search-vault.js` (query time) both refuse to run without. The gate is fail-closed: a missing or malformed deny file is a hard exit, not a silent empty list, so an index or a search you can't confirm is filtered never runs at all -- and it re-checks at query time too, so a stale `embeddings.json` built before a prefix was added still can't surface a denied path. Ships with an empty `deny_prefixes` array (a fresh install indexes everything by default); `index-deny.example.json` documents the pattern with illustrative placeholder paths, never real ones. The fail-closed logic lives in one shared module, `daemons/semantic-search/deny-list.mjs`, so both call sites can't drift out of sync on what "denied" means. See `skills/semantic-search/SKILL.md`. Tests: `daemons/tests/semantic-search-deny.test.mjs`, wired into CI.
