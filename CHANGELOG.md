...
### Added
- **Declared lifecycle extension (issue #49)**: an install supervised by an outside process can now attach that process's own completion handshake to the two lifecycle verbs by declaring it in `<target>/.aigent/lifecycle-extension.json` (`{"schema": "LifecycleExtension/v1", "resume_ack": "...", "capsule_ack": "..."}`), instead of forking `skills/resume/SKILL.md` or `skills/context-capsule/SKILL.md`. Forking was the only way to carry it before, and the installer treats a forked lifecycle skill as drift and quarantines it, so the next clear never announces and the supervising process holds the session forever. Both fields are optional; each is a single line of at most 500 characters, free of control characters, and may use `{capsule_id}` at most once. The declaration is data, never code: nothing loads a module, spawns a process, or teaches core any particular supervisor's vocabulary, so the same seam carries any protocol without core knowing one of them by name. `.aigent/` sits outside every installer-managed tree, so the declaration survives an install and an update with no preservation entry of its own. `daemons/resume-verb.mjs` renders `resume_ack` as step 5 of the injected procedure, after the core acknowledgement, and renders nothing at all when a `{capsule_id}` template has no loaded capsule to resolve against, so a degraded resume cannot invite a fabricated id. `capsule_ack` runs as step 6 of the capsule skill, after all core work and immediately **before** the terminal literal, deliberately not last: the literal has to be the final bytes in the transcript because the checkpoint measures the tail against `CHECKPOINT_TAIL_TOLERANCE_BYTES`, and a tool call landing after it spends margin the cycle does not have, so the capsule reads as stale and the session holds instead of clearing. Failure is fail-open and loud, the opposite of the namespace registry's fail-closed `process.exit(1)`: that registry gates indexing, where refusing to run is the safe direction, while this gates session start, where refusing to run wedges every clear on the session. An unusable declaration is refused whole, both fields go null, and one greppable line beginning `LIFECYCLE-EXTENSION: declaration ignored:` reaches both the procedure and the daemon error log; a partial declaration is never salvaged, because a half-armed handshake looks exactly like a working one. Both the declared fields and the refusal reason render through `inert()`, the existing single chokepoint for persisted text. The reason needs it too: V8's JSON parse error quotes the offending file's first bytes, so a file beginning with two newlines yields a message carrying a literal line break, which would otherwise let the declaration's author own a line inside the injected procedure. An install that declares nothing runs the unmodified standalone lifecycle. See [`docs/two-verb-lifecycle.md`](docs/two-verb-lifecycle.md). Tests: `daemons/tests/lifecycle-extension.test.mjs`.
...
### Changed
- **Fleet baseline recut to `FleetBaselineManifest/v7`**: `daemons/resume-verb.mjs` changed for the declared lifecycle extension above, so the sha256 pinned for it in `scripts/fleet-baseline-manifest.json` moved and every attestation verdict read the mismatch as tampering. That one entry is repinned; every other hash was recomputed and found unchanged. `daemons/lifecycle-extension.mjs` is not pinned, because the manifest's population rule admits what the settings template wires as a hook or statusLine plus the entry points `install.sh` names by path, and the new module is neither. `baseline_id` and `public_product_commit` stay at `867cb2ea`, which the attest pinned-commit case requires to be reachable, so those labels now lag the merges made since that cut.
