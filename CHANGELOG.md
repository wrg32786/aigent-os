...
### Changed
- **Fleet baseline recut to `FleetBaselineManifest/v7`**: `daemons/resume-verb.mjs` changed for the declared lifecycle extension above, so its pinned sha256 in `scripts/fleet-baseline-manifest.json` moved and every attestation verdict read the mismatch as tampering. All 32 pinned entries were recomputed from the cut commit: 1 moved, 31 unchanged, 0 missing. `baseline_id` and `public_product_commit` move with the schema, as in every prior recut. `daemons/lifecycle-extension.mjs` stays unpinned because the manifest's population rule admits what the settings template wires as a hook or statusLine plus the entry points `install.sh` names by path, and the new module is neither; the recut note recommends admitting it at the next release cut, on the `daemons/render_boundary.py` precedent.
