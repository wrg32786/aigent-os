...
- `/context-capsule`: reconciles, writes a capsule with the four required fields (plus any advisory prose), then stops. It stamps nothing itself; `validateCapsuleText()` (`daemons/capsule-verb.mjs`) is available as a self-check for the required fields; there is no pointer machinery.
- `/resume`: selects the newest valid capsule by `created_at`, re-grounds, and executes `next_valid_action`. The resume runtime flips the capsule `active → resumed` at load (`markCapsuleConsumed`, `daemons/lifecycle-common.mjs`) so a consumed capsule cannot be silently re-resumed; see [two-verb-lifecycle](two-verb-lifecycle.md) for the current contract. Resumption is still *proven* by the action taken, not by the status flip alone.
