...
> **Testing / harness isolation.** If you script or test against aigent-OS by spawning `claude` child sessions inside your vault directory, those children inherit the lifecycle hooks — their Stop autosaves will write *real capsules into your real vault*, and because `resume` loads the newest capsule, a test session's capsule can silently become your next session's "memory." Always point automated children at a throwaway root: set `AIGENT_ROOT` (first in every hook's root-resolution chain) to a scratch directory in the child's environment. One env var, total isolation.
